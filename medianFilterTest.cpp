        ...
        for(imCol = 4; imCol <= width -4; imCol++){
            ...
            for(wRPop = imRow + rowMod; wRPop <= imRow + 4; rowMod++){
                ...
                for(wCPop = imCol + colMod; wCPop <= imCol + 4; colMod++){          //...with those from the image according to a shifted index...
                    ...
                    cout << wRPop << endl << wCPop << endl << windex << endl;       //Print to see how many population iterations and incumbant data    
                    ...
                    if(windex == 80){
                        ...
                        cout << windex;         // are we hitting 81 (let alone 105) here!?!?
                    }

                    windex++;                   //try moving this outside one loop??  //This is to line up populating the next element in window Array.

                    if(windex > 80){
                        break;                  //IF we do wind up going above 80 then i'll break to the reset
                    }
